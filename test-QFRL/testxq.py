...
file3 = np.load('file3.npy', allow_pickle=True).tolist()
Deadend = np.load('Deadend.npy', allow_pickle=True).tolist()
buyable = np.load('buyable.npy', allow_pickle=True).tolist()
...
class CirModel(nn.Module):
    ...
    def forward(self, x):
        ...
        #
        # if is_batch == True:
        #     rst = (U @ x.permute(0,2,1)).permute(0,2,1)
        #     # print(rst.shape)
        #     rst = (rst.conj() @ op.PauliZ(self.n_qubits, 0).U_expand() @ rst.permute(0,2,1)).real
        #     rst = rst.squeeze(-1)
        #     # print(rst.shape)
        # else:
        #     rst = (U @ x.permute(1,0)).permute(1,0)
        #     rst = (rst.conj() @ op.PauliZ(self.n_qubits,0).U_expand() @ rst.permute(1,0)).real
        #     rst = rst.squeeze(-1)
